...
def libreoffice_convert(input_path, output_format, output_dir):
    ...
    # Set HOME to avoid LibreOffice user profile issues on servers
    env = os.environ.copy()
    if platform.system() != 'Windows':
        env['HOME'] = output_dir
    ...
    if not os.path.exists(out_file):
        ...
        if matches:
            ...
            raise RuntimeError(f"Output file not found. Files in dir: {files}")
...
def health():
    try:
        soffice = get_soffice_path()
        lo_status = f"LibreOffice found at: {soffice}"
    except Exception as e:
        lo_status = f"LibreOffice NOT found: {str(e)}"

    return jsonify({
        'status': 'ok',
        'message': 'ಕನ್ನಡ PDF API ಚಾಲನೆಯಲ್ಲಿದೆ',
        'libreoffice': lo_status,
        'tools': ['word-to-pdf', 'pdf-to-word', 'pdf-merge', 'pdf-split', 'img-to-pdf']
    })
